           ...
           for index = 2:length(vector) %testa för alla möjliga värden index kan ha
               if val > vector(index-1) %för att hitta position där inputvärdet är mellan två värden och det tidigare värdet i vektorn är mindre än det senare.
                   ...
                   er2=min(abs(val-vector(index)));
                    disp(['The value you put in is to be found between position ',num2str(index-1),  ' and ',num2str(index),' and are ',num2str(er), ' bigger then the value (',num2str(vector(index-1)), ') in position ',num2str(index-1),', and ',num2str(er2), ' smaller then the value (',num2str(vector(index)), ') in position ',num2str(index+1) ])
                    end
               elseif val > vector(index) %för att hitta position där inputvärdet är mellan två värden och det tidigare värdet i vektorn är mindre än det senare. 
                   ...
                   er=min(abs(val-vector(index-1)));
                   er2=min(abs(val-vector(index)));
                    disp(['The value you put in is to be found between position ',num2str(index-1),  ' and ',num2str(index),' and are ',num2str(er), ' smaller then the value (',num2str(vector(index-1)), ') in position ',num2str(index-1),', and ',num2str(er2), ' bigger then the value (',num2str(vector(index)), ') in position ',num2str(index+1) ])
                    end
               end
           end
    end
